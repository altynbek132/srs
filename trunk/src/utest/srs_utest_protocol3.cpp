...
VOID TEST(ProtocolRtpTest, SrsRtpVideoBuilderPackageStapA)
{
    ...
    // Test package_stap_a - may fail due to format requirements but shouldn't crash
    srs_error_t stap_err = builder.package_stap_a(msg, pkt);
    srs_freep(stap_err);
...
VOID TEST(ProtocolRtpTest, SrsRtpVideoBuilderPackageNalusWithFormat)
{
    ...
    // Create NALU samples for testing
    std::vector<SrsNaluSample *> samples;
    SrsNaluSample *sample1 = new SrsNaluSample();
    sample1->bytes_ = video_data + 5;
    sample1->size_ = 5;
    samples.push_back(sample1);

    SrsNaluSample *sample2 = new SrsNaluSample();
    sample2->bytes_ = video_data + 10;
    sample2->size_ = 5;
    samples.push_back(sample2);

    std::vector<SrsRtpPacket *> pkts;

    // Test package_nalus with format - may fail due to format validation but shouldn't crash
    srs_error_t nalus_err = builder.package_nalus(msg, samples, pkts);
    srs_freep(nalus_err);
...
VOID TEST(ProtocolRtpTest, SrsRtpVideoBuilderPackageFuA)
{
    ...
    video_data[0] = 0x17; // keyframe + AVC
    video_data[1] = 0x67; // SPS NALU header
    for (int i = 2; i < 100; i++) {
        video_data[i] = i;
    }
    ...
    SrsNaluSample *sample = new SrsNaluSample();
    sample->bytes_ = video_data + 1; // Start from NALU header
    sample->size_ = 80;              // Large enough to require fragmentation

    std::vector<SrsRtpPacket *> pkts;
    int fu_payload_size = 30; // Small payload size to force fragmentation

    // Test package_fu_a - may fail due to format requirements but shouldn't crash
    srs_error_t fu_err = builder.package_fu_a(msg, sample, fu_payload_size, pkts);
    srs_freep(fu_err);
